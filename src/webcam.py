...
out = open('webcam_profile.txt', 'w')

out.writelines('This text file contains all device data for searching exposed IP camera \n')
out.writelines('The result will be shown in a format as:\n*\neach device\'s attributes => \ndescription\n* \none by one \n')
out.write('\n')

print 'This console shows all device data for searching exposed IP camera '
print 'The result will be shown in a format as:\n*\neach device\'s attributes => \ndescription\n* \none by one'

str200 = '200 OK'
str401 = '401 Unauthorized'
str_no_cache = 'no-cache'
str_no_store = 'no-store'
...
try:
                ...
                for v in value:              
                    ...
                    for k1 in v :                       
                        ...
                        if type(k1) is unicode:
                            ...
                            out.write(str(k1) + ' => \n')
                        if k1 == 'data':
                            if v[k1].find(str200) > 0 :
                                count_200 += 1
                            elif v[k1].find(str401) > 0 :
                                count_401 += 1
                            if v[k1].find(str_no_cache) > 0 and v[k1].find(str_no_store) > 0 :
                                count_nc_ns += 1
                            elif v[k1].find(str_no_cache) > 0 :
                                count_nc += 1
    ...
    print 'among ' + str(count) + ' devices, there are : \n '
    print str(count_200) + ' devices that with http header :' + str200
    print str(count_401) + ' devices that with http header :' + str401
    
    out.write('among ' + str(count) + ' devices, there are : \n')
    out.write(str(count_200) + ' devices with http header :' + str200 + '\n')
    out.write(str(count_401) + ' devices with http header :' + str401 + '\n')
    
    print 'for cache control to prevent user info leakage, ' 
    print str(count_nc_ns) + ' devices applies both no-store and no-cache header'
    print str(count_nc) + ' devices applies only no-cache header'
    
    out.write('\nfor cache control to prevent user info leakage, \n')
    out.write(str(count_nc_ns) + ' devices applies both no-store and no-cache header \n')
    out.write(str(count_nc) + ' devices applies only no-cache header \n')
